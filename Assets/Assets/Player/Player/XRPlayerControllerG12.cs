...
namespace Prefabs.Player
{
    ...
    /// </summary>
    public class XRPlayerControllerG12 : MonoBehaviour
    {
        ...
        public GameObject telePearlPrefab;
        ...
        private void Awake()
        {
            if(dontDestroyOnLoad)
            {
                ...
                MonoBehaviourExtensions.DontDestroyOnLoadGameObjects.Add(gameObject);
            }
        ...
        private void Start()
        {
            foreach (var item in GetComponentsInChildren<TeleportRayG12>())
            {
                item.OnTeleport.AddListener(OnTeleport);
            }
        }

        public void OnTeleport(Vector3 position, Vector3 launchVector)
        {
            var telePearl = Instantiate(telePearlPrefab, position, Quaternion.identity);
            telePearl.GetComponentInChildren<Assets.Player.Player.TelePearl.TelePearl>().OnPearlLand.AddListener(OnPearlLand);
            telePearl.GetComponent<Rigidbody>().velocity = launchVector * TeleportRayG12.launchSpeed;
            // Debug.Log("Invoked OnTeleport");
        ...
        public void OnPearlLand(Vector3 position)
        {
            userLocalPosition = transform.InverseTransformPoint(headCamera.transform.position);
            userLocalPosition.y = 0;

            var movement = position - transform.TransformPoint(userLocalPosition);
            transform.position += movement;
        }
