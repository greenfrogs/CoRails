    ...
    public class BridgeVisitor : ChunkVisitor {
        ...
        public override void Run(Chunk chunk) {
            ...
            while (true) {
                for (int i = 0; i < 4; i++) {
                    ...
                    if (scanChunk != null && scanChunk.type == BaseLayerType.Grass) {
                        founds[i] = true;
                    }
                }
            ...
            if (vertical) {
                ...
                // As these bridges are rotated, they need to be offset by +1
        ...
        private bool Scan(Chunk chunk, int range) {
            ...
            for (int i = 0; i < range / 2; i++) {
                if (scanner.north.north != null) {
                    scanner = scanner.north;
                }

                if (scanner.west.west != null) {
            ...
            for (int i = 0; i < range; i++) {
                for (int j = 0; j < range; j++) {
                    if (scanner.properties.ContainsKey("Bridge")) {
                        return true;
                    }
