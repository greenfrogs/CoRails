...
public class Scoring : MonoBehaviour, INetworkComponent, INetworkObject
{
    ...
    private void SendScore(IPeer newPeer)
    {
        ...
        var doSend = roomClient.Peers.Select(peer => peer.UUID.Last()).All(peerSuffix => peerSuffix > mySuffix);
    ...
    public void ProcessMessage(ReferenceCountedSceneGraphMessage message)
    {
        ...
        if (scoringMessage.TargetUuid == roomClient.Me.UUID)
        {
            ...
            {
                foreach (var (eventType, eventScore) in scoringMessage.ScorePerType)
                {
